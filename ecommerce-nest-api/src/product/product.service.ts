...
export class ProductService {
  ...
  //read one product instead of whole list above
  public findOne(index: number): any {
    return this.productList[index];
  }

  //update one item
  public update(dto: CreateProductDto, id: string): Product[] {
    return this.productList.map((product) => {
      return id === product.uniqueId ? dto : product;
    });
  }

  public create(dto: CreateProductDto): Product[] {
    const newList = [...this.productList, dto];
    return newList;
  }

  public remove(id: string) {
    const newList = this.productList.filter(
